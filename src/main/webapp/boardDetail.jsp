		...
		$('#commentsAdd').append("<div class='comments' id='newComments'>"+
					"<div class='comments-heading'>"+
						"<div id='reply_num_and_give_medal_area'>"+
							...
							"<span id='comment_writer_${member.id }'>작성자 : ${member.id }</span> <span id='comment_date'> 작성일 : "+date+"</span>"+
						"</div>"+
					"</div>"+
            ...
            success : function(commentData) {
            	$('#newComments').replaceWith(
            			...
            			"<span id='give_medal' onclick='cmedal("+commentData.ccode+",\""+commentData.writerid+"\")'>메달 주기</span>"+
									...
									"<span id='comment_writer_"+commentData.writerid+"'>작성자!!! : "+commentData.writerid+"</span>"+
									"</div>"+
									"</div>"+
	...
	function wcomment(code)
	{
		...
		parentComment.after("<div class=recomments id='newRecomments'>"+
				...
				"<div id='reply_num_and_give_medal_area'>"+
					...
					"<span id='comment_writer_${member.id }'>작성자 : ${member.id }</span> <span id='comment_date'>작성일 : "+date+
				"</span></div>"+
			"</div>"+
			"<div class='comments-body'>"+
			"<textArea id='newCArea'></textArea>"+
			...
			<div class="boardDetail">
				<div class="boardDetail-header">
					<div class="boardDetail_title">
						<h2>${board.title}&nbsp;&nbsp;&nbsp;<a href="#"><span
								id="posted_from">${board.code.name }게시판</span></a>
						</h2>
					</div>
					<br>
					...
					<div class="boardDetail_date">
						...
						<c:if test="${board.point.medal eq 0}">
							<span class="current_medal_number" id="pmdiv"
								style="display: none;"> 게시글 메달 갯수 :<span id="bmedal">
									${board.point.medal}</span>
							</span>
						</c:if>
								...
								<div class='comments-heading'>
									<div id='reply_num_and_give_medal_area'>
										<c:if test="${c.lev eq 1}">
											<span id="reply_number" class="commentNumber">${num }번째
												댓글</span>
											<c:set var="cnum" value="1" />
											<c:set var="num" value="${num+1 }" />
										</c:if>
										...
										<c:if test="${c.lev eq 2}">
											<span id="reply_number" class="commentNumber">${num-1}-${cnum }번째
												대댓글</span>
											<c:set var="cnum" value="${cnum+1}" />
										</c:if>
										<span id='give_medal'><strike>메달 주기</strike></span>
										<span id="comment_writer">작성자 :
											${c.writerid}</span>
									</div>
									<div class='comment_authordate'>
							...
							<div class="comments" id="pa${c.ccode }">
								<div class="comments-heading">
									...
									<div id="reply_num_and_give_medal_area">
										...
										<span id="give_medal"
											onclick="cmedal(${c.ccode},'${c.writerid }')">메달 주기</span> 
											<c:if test="${sWriter eq c.writerid}">
											<span id="comment_writer_${sWriter}">작성자 : ${c.writerid } </span>
											</c:if>
								...
								<div class="comments-footer">
									...
									<div class="comment_edit_add">
										<button id="comment_edit"
											onclick="beforeCEdit(${c.ccode },$(this).parent().parent().prev().prev().find('span[id=\'reply_number\']').text())">수정하기</button>
										<c:if test="${c.lev ne 2}">
